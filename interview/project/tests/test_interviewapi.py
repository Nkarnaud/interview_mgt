...
from project.tests.utils import get_interviewer, get_candidate
...
class TestInterviewService(BaseTestCase):
    ...
    def test_interviewer_list(self):
        ...
        with self.client:
            ...
            self.assertIn('Irene',
                          data['data']['interviewers'][0]['firstname'])
            self.assertIn('Prinz',
                          data['data']['interviewers'][0]['lastname'])
            self.assertIn('prinz@chemondis.com',
                          data['data']['interviewers'][0]['email'])
            self.assertIn('Test',
                          data['data']['interviewers'][1]['firstname'])
            self.assertIn('test',
                          data['data']['interviewers'][1]['lastname'])
            self.assertIn('test@chemondis.com',
                          data['data']['interviewers'][1]['email'])
            self.assertIn('success', data['status'])
    ...
    def test_interviewer_update_with_empty_json(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('Sorry. The interviewer does not exists',
                          data['message'])
            self.assertIn('fail', data['status'])
    ...
    def test_interviewer_delete(self):
        ...
        interviewer = get_interviewer("prinz@chemondis.com")
        with self.client:
            resp = self.client.delete(f'/interviewer/delete/{interviewer.id}')
    ...
    def test_candidate_details(self):
        ...
        with self.client:
            ...
            self.assertIn("arnaud.tsombeng@chemondis.com",
                          data['data']['email'])
            self.assertIn('success', data['status'])
    ...
    def test_candidate_list(self):
        ...
        with self.client:
            ...
            self.assertIn('test25',
                          data['data']['candidates'][0]['firstname'])
            self.assertIn('test462',
                          data['data']['candidates'][0]['lastname'])
            self.assertIn('testrfd@chemondis.com',
                          data['data']['candidates'][0]['email'])
            self.assertIn('Test',
                          data['data']['candidates'][1]['firstname'])
            self.assertIn('test',
                          data['data']['candidates'][1]['lastname'])
            self.assertIn('test@chemondis.com',
                          data['data']['candidates'][1]['email'])
            self.assertIn('success', data['status'])
    ...
    def test_candidate_update_with_empty_json(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('Sorry. The candidate does not exist',
                          data['message'])
            self.assertIn('fail', data['status'])
    ...
    def test_delete_not_exist_candidate(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('Sorry. The candidate does not exists',
                          data['message'])
            self.assertIn('fail', data['status'])
    ...
    def test_creation_of_duplicate_interview(self):
        add_interview("Backend dev interview",
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('This interview slot already exit.',
                          data['message'])
            self.assertIn('fail', data['status'])
    ...
    def test_get_interview_detail(self):
        ...
        with self.client:
            ...
            self.assertIn('Sat, 09 Feb 2019 09:00:00 GMT',
                          data['data']['start_time'])
            self.assertIn('Sat, 09 Feb 2019 10:00:00 GMT',
                          data['data']['end_time'])
            self.assertIn('success', data['status'])
    ...
    def test_get_interview_list(self):
        add_interview("Backend dev interview",
                      "02-09-2019:09:00", "02-09-2019:10:00")
        add_interview("Frontend dev interview",
                      "02-09-2019:09:00", "02-09-2019:10:00")
        with self.client:
            ...
            self.assertIn('Backend dev interview',
                          data['data']['interviews'][0]['title'])
            self.assertIn('Sat, 09 Feb 2019 09:00:00 GMT',
                          data['data']['interviews'][0]['start_time'])
            self.assertIn('Sat, 09 Feb 2019 10:00:00 GMT',
                          data['data']['interviews'][0]['end_time'])
            self.assertIn('Frontend dev interview',
                          data['data']['interviews'][1]['title'])
            self.assertIn('Sat, 09 Feb 2019 09:00:00 GMT',
                          data['data']['interviews'][1]['start_time'])
            self.assertIn('Sat, 09 Feb 2019 10:00:00 GMT',
                          data['data']['interviews'][1]['end_time'])
            self.assertIn('success', data['status'])
    ...
    def test_delete_interview_wrong_id(self):
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('Sorry. The interview does not exists',
                          data['message'])
            self.assertIn('fail', data['status'])

    # Test get empty interview slot
    ...
    def test_candidate_assigned_interview(self):
        interview = add_interview("Backend dev interview",
                                  "02-09-2019:09:00", "02-09-2019:10:00")
        candidate = add_candidate("test25", "test462",
                                  "test@chemondis.com")
        with self.client:
            response = self.client.post(
                f'/candidate_assign/{candidate.id}/{interview.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 201)
            self.assertIn(
    ...
    def test_thesame_candidate_assigned_thesame_interview(self):
        inter = add_interview("Backend dev interview",
                              "02-09-2019:09:00", "02-09-2019:10:00")
        cand = add_candidate("test25", "test462", "test@chemondis.com")
        with self.client:
            resp = self.client.post(f'/candidate_assign/{cand.id}/{inter.id}')
            resp = self.client.post(f'/candidate_assign/{cand.id}/{inter.id}')
            data = json.loads(resp.data.decode())
    ...
    def test_candidate_assigned_interview_with_candidate(self):
        inte = add_interview("Backend dev interview",
                             "02-09-2019:09:00", "02-09-2019:10:00")
        cand = add_candidate("test25", "test462", "test@chemondis.com")
        cand1 = add_candidate("arnaud", "Tsomben", "arnaud@chemondis.com")
        with self.client:
            resp = self.client.post(f'/candidate_assign/{cand.id}/{inte.id}')
            resp = self.client.post(f'/candidate_assign/{cand1.id}/{inte.id}')
            data = json.loads(resp.data.decode())
            self.assertEqual(resp.status_code, 400)
            self.assertIn(
    ...
    def test_interviewer_assign_to_interview(self):
        inter = add_interview("Backend dev interview",
                              "02-09-2019:09:00", "02-09-2019:10:00")
        intere = add_interviewer("Irene", "Pri<nz", "prinz@chemondis.com")
        with self.client:
            resp = self.client.post(
                f'/interviewer_assign/{intere.id}/{inter.id}')
            data = json.loads(resp.data.decode())
    ...
    def test_interviewer_assign_to_many_interview(self):
        inter = add_interview("Backend dev interview",
                              "02-09-2019:09:00", "02-09-2019:10:00")
        inter1 = add_interview("Frontend dev interview",
                               "02-09-2019:09:00", "02-09-2019:10:00")
        intere = add_interviewer("Irene", "Pri<nz", "prinz@chemondis.com")
        with self.client:
            response = self.client.post(
                f'/interviewer_assign/{intere.id}/{inter.id}')
            response = self.client.post(
                f'/interviewer_assign/{intere.id}/{inter1.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 201)
            self.assertIn(
                          'This interviewer has been assigned to interview.',
    ...
    def test_interviewer_assign_to_thesame_interview(self):
        ...
        with self.client:
            ...
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 400)
            self.assertIn('This interviewer is already assigned to interview.',
                          data['message'])
            self.assertIn('Fail', data['status'])
    ...
    def test_interviewers_interview_list(self):
        interview = add_interview("Backend dev interview",
                                  "02-09-2019:09:00", "02-09-2019:10:00")
        interview1 = add_interview("Frontend dev interview",
                                   "02-09-2019:09:00", "02-09-2019:10:00")
        interviewer = add_interviewer(
            "Irene", "Pri<nz", "prinz@chemondis.com")
        with self.client:
            response = self.client.post(
                f'/interviewer_assign/{interviewer.id}/{interview.id}')
            response = self.client.post(
                f'/interviewer_assign/{interviewer.id}/{interview1.id}')
            response = self.client.get(
                f'/interview/interviewer/list/{interviewer.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertEqual(len(data['data']['interviews']), 2)
            self.assertIn('Backend dev interview',
    ...
    def test_get_candidate_interview_detail(self):
        ...
        with self.client:
            response = self.client.post(
                f'/candidate_assign/{candidate.id}/{interview.id}')
            response = self.client.get(
                f'/interview/candidate/{candidate.id}')
            data = json.loads(response.data.decode())
            self.assertEqual(response.status_code, 200)
            self.assertEqual(len(data['data']['interview']), 1)
            self.assertIn('Backend dev interview',
                          data['data']['interview'][0]['title'])
            self.assertIn('Sat, 09 Feb 2019 09:00:00 GMT',
                          data['data']['interview'][0]['start_time'])
            self.assertIn('Sat, 09 Feb 2019 10:00:00 GMT',
                          data['data']['interview'][0]['end_time'])
            self.assertIn('success', data['status'])
